...
class MCF88LW12TER(MessageConverter):
    ...
    def _convert(self, payload, port) :
        ...
        try:
            ...
            # Handle measurement packets
                ...
                publ_array.append(entry)
                # time2
                time = self.__toTime(payload[11:15])
                 # temperature_2
                value = (payload[16]<<24>>16 | payload[15]) / 100
                entry = {}
                entry["sensorid"] = "temperature"
                entry["ts"] = time
                entry["value"] = value
                ...
                publ_array.append(entry)
                # pressure2
                value = (payload[20] << 16) | (payload[19] << 8) | payload[18]
                entry = {}
                entry["sensorid"] = "pressure"
                entry["ts"] = time
                entry["value"] = value
                publ_array.append(entry)
                # time3
                time = self.__toTime(payload[21:25])
                 # temperature_3
                value = (payload[26]<<24>>16 | payload[25]) / 100
                entry = {}
                entry["sensorid"] = "temperature"
                entry["ts"] = time
                entry["value"] = value
                ...
                publ_array.append(entry)
                # pressure3
                value = (payload[30] << 16) | (payload[29] << 8) | payload[28]
                entry = {}
                entry["sensorid"] = "pressure"
                entry["ts"] = time
                entry["value"] = value
                ...
                self.logger.debug("unknown payload type")
